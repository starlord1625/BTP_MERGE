...
def segment_time_series(data_dict,
    ...
    sampling_delta_norm = np.diff(tf_pd['abs_time']).mean() #time between two samples in time-series
    ...
    timedeltas = np.array([td.total_seconds() for td in timedeltas])   # number of seconds between prev event and present event
    abs_times = event_tf_norm[:, 0]   #abs_time of event sequence : event_tf_norm['abs_time'] :shape = (event_l_seq,)
...
class TsEventDataset(Dataset):
    def __init__(self, data_dict, num_types, X_context, y_horizon, window_skip, train_prop=0.8,
        ...
        self.x_dim = self.dataset['data_pairs'][0]['X_ts'].shape[1]   # number of features in time-series
        self.t_dim = self.dataset['data_pairs'][0]['X_tf'].shape[1]   # len of feature_set
